  ...
  return (
    <section className="research-results" aria-labelledby="research-results-title">
      <div className="research-results__heading">
        <div><span className="eyebrow">ARASTIRMACI / SONUCLAR</span><h1 id="research-results-title">Sifreli toplamdan istatistige</h1><p>Hicbir bireysel deger cozulmez; yalnizca grup kontenjans hucreleri ve (n, sum, sumSq) toplamlari kullanilir.</p></div>
        <button className="pill pill--ghost" disabled={loading || wrongNetwork} onClick={() => void refresh()}>{loading ? "Okunuyor..." : "Yenile"}</button>
      </div>
      {wrongNetwork && <div className="notice notice--warn">Sonuclar yalnizca Sepolia aginda okunabilir.</div>}
      {notice && <div className={`notice notice--${notice.kind}`} role="status">{notice.text}</div>}
      {!loading && !open && <div className="card card--bone"><h2>Cozulmus acik sorgu yok</h2><p className="card__body">Sorgular ekraninda acilim yetkisi verildikten sonra burada analiz yapabilirsiniz.</p></div>}
      ...
      {disclosure?.executed && <div className="research-results__action card card--bone"><div><span className="eyebrow">SORGU #{open?.queryId}</span><h2>Grup toplamini coz ve hesapla</h2><p>Ki-kare ve BH-FDR genomik tablolarda; Welch t ve Cohen d biyobelirtec tablolarinda gosterilir.</p></div><button className="pill pill--primary" disabled={analyzing || !signer} onClick={() => void analyze()}>{analyzing ? "Cozuluyor..." : "Toplamlari coz ve analiz et"}</button></div>}
      {snps && <article className="research-results__table card"><div className="card__head"><h2>Genomik sonuc</h2><span className="eyebrow">KI-KARE + BH-FDR</span></div><div className="table"><div className="table__head table__head--gwas"><span>VARYANT</span><span>KONTROL 0/1/2</span><span>VAKA 0/1/2</span><span>CHI2</span><span>p</span><span>p (FDR)</span></div>{snps.map((result) => <div className={`table__row table__row--gwas${result.pAdjusted < 0.05 ? " is-hit" : ""}`} key={result.snp}><span className="mono">{result.rsid}</span><span className="mono">{result.table[0].join(" / ")}</span><span className="mono">{result.table[1].join(" / ")}</span><span className="mono">{Number.isFinite(result.chi2) ? result.chi2.toFixed(2) : "-"}</span><span className="mono">{formatP(result.p)}</span><span className="mono">{formatP(result.pAdjusted)}</span></div>)}</div>{snps.some((result) => !result.reliable) && <div className="notice notice--warn">Bazı varyantlarda beklenen hucre sayisi 5'in altinda; ki-kare yaklasimi guvenilir degil.</div>}</article>}
      {metrics && metrics.length > 0 && <article className="research-results__table card"><div className="card__head"><h2>Biyobelirtec sonucu</h2><span className="eyebrow">WELCH t + COHEN d</span></div><div className="table"><div className="table__head table__head--welch"><span>METRIK</span><span>KONTROL</span><span>VAKA</span><span>t / d</span><span>p</span></div>{metrics.map((result) => <div className="table__row table__row--welch" key={result.metric}><span className="mono">{result.code}</span><span className="mono">{result.control.n ? `${result.control.mean.toFixed(2)} ± ${result.control.sd.toFixed(2)} (n=${result.control.n})` : "-"}</span><span className="mono">{result.cases.n ? `${result.cases.mean.toFixed(2)} ± ${result.cases.sd.toFixed(2)} (n=${result.cases.n})` : "-"}</span><span className="mono">{Number.isFinite(result.t) ? `${result.t.toFixed(2)} / ${result.cohensD.toFixed(2)}` : "-"}</span><span className="mono">{formatP(result.p)}</span></div>)}</div>{metrics.some((result) => result.note) && <div className="notice notice--info">Bazı metriklerde her grupta en az iki katilimci olmadigi icin Welch t-testi hesaplanamiyor.</div>}</article>}
      {snps && open && <button className="pill pill--primary research-results__settle" disabled={settling || !signer} onClick={() => void settle()}>{settling ? "Dagitiliyor..." : "Analizi onayla ve odemeyi dagit"}</button>}
    </section>
  );
}
